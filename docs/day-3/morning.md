...
!!! info "There is no Installation or Erection Status Link in PowerFab 2026"
    ...
    A task with no matching event — Installation is the usual case — needs **Status Link = [None]** and **Status Summary Method = No Factor**. Do not force-reuse an unrelated link to make the field look populated. The full walkthrough is at [Implementation Step 2](../implementation/part-1/step-02.md).
